...
데이터프레임의 **행 인덱스**와 **열 이름**은 위의 `index`와 `columns` 속성을 이용해서 수정할 수 있다.
...
###  <span style="color:navy">행 선택<span>
...
두 메소드를 이용하여 **단일 행**을 선택할 경우 다음과 같으며, 
...
**특정 행만 선택**하고 싶을 경우에는 **리스트**를 이용한다. 
...
**슬라이싱**을 이용하여 행을 선택할 경우에는 `loc`와 `iloc`가 선택하는 범위에 차이가 있는 것을 확인할 수 있다.
...
###  <span style="color:navy">행 추가/수정<span> 
...
### <span style="color:navy">원소 선택/수정<span>
...
###  <span style="color:navy">열 선택<span>

데이터프레임에서 열을 선택할 때는 `[](대괄호)`를 이용하거나, 메소드처럼 `.(도트)`를 이용하여 열 이름을 선택할 수 있다. 단, `.(도트)`를 이용한 방법의 경우 단일 열을 선택할 때만 사용한다. 
...
### <span style="color:navy">열 추가/수정<span>
