...
---
#### Contents
- [양방향 연결 리스트 (Doubly Linked Lists)](#양방향-연결-리스트-doubly-linked-lists)
- [양방향 연결 리스트 핵심 연산](#양방향-연결-리스트-핵심-연산)
  - [(1) 리스트 순회 (Traversal/Reverse)](#1-리스트-순회-traversalreverse)
  - [(2) N번째 특정 원소 탐색](#2-N번째-특정-원소-탐색)
  - [(3) 원소 삽입 (Insertion)](#3-원소-삽입-insertion)
  - [(4) 원소의 삭제 (Deletion)](#3-원소의-삭제-deletion)
  - [(5) 두 리스트 병합 (Concatenation)](#4-두-리스트-병합-concatenation)
...
- 양방향 연결 리스트는 이전 연결 리스트와 달리 양방향으로 진행이 가능하기 때문에 이전 Node와 달리 <u>이전(***prev***) 노드에 대한 주소를 할당하는 포인터를 추가할 필요</u>가 있다. 따라서, 기존 Node클래스에 `self.prev`<u>인스턴스 변수를 추가</u>한다.
